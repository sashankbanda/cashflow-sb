...
- **Dot-matrix numerals** (`Doto`, heavy weight) are reserved for *hero* numbers only: net position on Home, group total, settle amount. Everything else uses SF/Inter. This is the signature look from the references (LED-style `09:41`).
...
- Max ~2 gradient panels visible per viewport; the rest is quiet glass. (References look rich because saturation is *rationed*.)
