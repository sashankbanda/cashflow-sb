...
### DG.6 — UPI quick capture (share target + paste) and personal-entry edit
- **Decision:** (1) PWA registers a **share target** (`manifest.share_target` → GET `/add`): sharing a UPI receipt/bank SMS into Cashflow opens `/add` with the text, `lib/upi-parse.ts` (unit-tested: GPay/PhonePe/bank-SMS/credit formats, VPA cleanup, title-casing) prefills amount + payee + direction; the user picks a category and saves through the same outbox path. (2) The same `/add` screen has a **Paste a payment message** button (clipboard read on tap) — the iOS path, since **Safari has no share-target support**; linked from Money → "Quick add from a UPI message". (3) Full personal CRUD: `updatePersonalExpenseAction` + `updatePersonalExpense` service (owner-only, personal-only; payer/split rows updated in lockstep with the amount) and `PersonalEntrySheet` — tapping a personal ledger row now opens edit (amount/description/category/date/direction) + delete, replacing the delete-only sheet.
- **Reasoning:** truly automatic capture is impossible in a browser (no SMS/notification access) — the share/paste flow is the honest 90%: the amount and payee enter themselves, the user only answers "why". Auto-capture proper needs the Capacitor Android shell (logged as the next step if wanted). Edit was the one missing CRUD op (groups already had it).
- **Rejected:** claiming background auto-capture in the PWA (impossible); a POST share target (GET keeps `/add` a plain server-rendered page); allowing tag edits in the entry sheet (scope — tags editable by re-creating; follow-up).

### DG.7 — iPhone zero-touch capture (Shortcuts webhook) + haptic keypad
- **Decision:** (1) `users.capture_token` (migration 0007) + `POST /api/capture` `{token, text}`: an **iOS Shortcuts automation** ("Message contains 'debited'" → Get Contents of URL, background, no app open) delivers each bank SMS; server parses via `upi-parse`, saves a personal entry under the first system category with a **deterministic sha256-as-UUID idempotency key** (per user+text — automation retries can't double-enter), dates it in the user's timezone, and pushes "tap to set a category". Token-authed, 20/min/IP limited, rotate/revoke from **Profile → Auto-capture payments** (`/settings/capture`, with copyable URL/token/body + both Shortcut recipes). (2) Keypad keys grown to 64px (display-face digits, pressed scale+fill), haptic on pointer-down; `useHaptics` gains the **iOS native-switch Taptic fallback** (hidden `input[switch]` click — Safari 17.4+) since Safari lacks the Vibration API, upgrading every existing haptic call site on iPhone at once.
- **Reasoning:** this is the fastest possible iPhone flow without an App Store app — the SMS books itself and the user only categorizes from a notification. Reuses the parser, personal-expense service, idempotency, and push infra wholesale. The switch-click trick is the only known way to fire the Taptic engine from a web page.
- **Rejected:** paste-only for iOS (owner explicitly wants faster); OTP-style SMS APIs (only for origin-bound codes); storing raw SMS server-side (parse-and-discard — only the derived entry persists).
